...
public class OffresController {
    ...
    private VBox createCard(OffreEmploi offre) {
        ...
        description.getStyleClass().add("job-description");
        ...
        card.getChildren().addAll(header, title, description, separator, footer);
    ...
    private void showDetailContent(OffreEmploi offre) {
        ...
        // Tags removed from header - only show in detail body
        
        titleSection.getChildren().add(jobTitle);
        
        VBox metaInfo = new VBox(8);
        metaInfo.setStyle("-fx-padding: 5 0 0 0;");
        Label contractType = new Label("▸ " + offre.getTypeContrat());
        Label status = new Label("● " + offre.getStatutOffre());
        contractType.getStyleClass().add("detail-meta-label");
        status.getStyleClass().add("detail-meta-label");
        metaInfo.getChildren().addAll(contractType, status);
        
        topHeader.getChildren().addAll(titleSection, metaInfo);
        ...
        if (!criteres.isEmpty()) {
            for (CritereOffre critere : criteres) {
                ...
                if (critere.getNiveauExperience() != null && !critere.getNiveauExperience().trim().isEmpty()) {
                    ...
                    expIcon.setStyle("-fx-font-size: 16px; -fx-padding: 0 10 0 0;");
                ...
                if (critere.getNiveauEtude() != null && !critere.getNiveauEtude().trim().isEmpty()) {
                    ...
                    eduIcon.setStyle("-fx-font-size: 16px; -fx-padding: 0 10 0 0;");
                ...
                if (critere.getCompetencesRequises() != null && !critere.getCompetencesRequises().trim().isEmpty()) {
                    ...
                    for (String comp : competences) {
                        ...
                        compIcon.setStyle("-fx-font-size: 14px; -fx-padding: 0 12 0 0; -fx-text-fill: #6366f1; -fx-font-weight: bold;");
