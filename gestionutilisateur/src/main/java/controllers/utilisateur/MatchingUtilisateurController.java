...
import services.candidat.MatchingService;
...
public class MatchingUtilisateurController implements Initializable {
    ...
    private int idUtilisateurConnecte = -1;  // ✅ CHANGÉ : -1 au lieu de 3
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        if (userConnecte != null) {
            idUtilisateurConnecte = userConnecte.getId_utilisateur();
            System.out.println("✅ [MATCHING] Utilisateur en session : ID=" + idUtilisateurConnecte);
        } else {
            System.out.println("❌ [MATCHING] Aucun utilisateur en session !");
            lblProfilStatus.setText("❌ Erreur : Aucun utilisateur connecté");
            return;
        }
    ...
    private void chargerMatchings() {
        if (idUtilisateurConnecte <= 0) {
            System.err.println("❌ ID utilisateur invalide : " + idUtilisateurConnecte);
            afficherChargement(false);
            afficherEtatVide();
            lblProfilStatus.setText("❌ Erreur : ID utilisateur invalide");
            return;
        }

        System.out.println("\n=== CALCUL MATCHINGS ===");
        System.out.println("Pour l'utilisateur ID=" + idUtilisateurConnecte);
