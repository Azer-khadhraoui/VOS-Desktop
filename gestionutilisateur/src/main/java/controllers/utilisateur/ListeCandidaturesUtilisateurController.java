...
import entities.Candidature;  // ✅ AJOUT
...
import services.candidat.PdfService;
...
public class ListeCandidaturesUtilisateurController implements Initializable {
    ...
    private void chargerDonnees() {
        ...
        // ✅ Étape 2 : Vérifier l'ID avant de charger
        System.out.println("\n=== CHARGEMENT CANDIDATURES ===");
        System.out.println("ID utilisateur courant : " + idUtilisateurCourant);

        // Récupère TOUTES les candidatures d'abord
        List<Candidature> toutesLes = service.getAll();
        System.out.println("Total candidatures en BD : " + toutesLes.size());
        // Filtrer par utilisateur
        toutesLesCandidatures = toutesLes.stream()
                .filter(c -> {
                    System.out.println("  - Candidature #" + c.getIdCandidature()
                            + " : id_utilisateur=" + c.getIdUtilisateur()
                            + " (cherche " + idUtilisateurCourant + ")");
                    return c.getIdUtilisateur() == idUtilisateurCourant;
                })
                .collect(Collectors.toList());

        System.out.println("Candidatures trouvées pour ID " + idUtilisateurCourant + " : "
                + toutesLesCandidatures.size());
