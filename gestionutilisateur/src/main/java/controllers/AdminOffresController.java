...
public class AdminOffresController {
    ...
    @FXML
    private TableColumn<OffreEmploi, Integer> colIdOffre;
    @FXML
    private TableColumn<OffreEmploi, String> colTitre;
    @FXML
    private TableColumn<OffreEmploi, String> colDescription;
    @FXML
    private TableColumn<OffreEmploi, String> colTypeContrat;
    @FXML
    private TableColumn<OffreEmploi, String> colWorkPreference;
    @FXML
    private TableColumn<OffreEmploi, String> colLieu;
    @FXML
    private TableColumn<OffreEmploi, String> colStatutOffre;
    @FXML
    private TableColumn<OffreEmploi, Date> colDatePublication;
    @FXML
    private TableColumn<OffreEmploi, Integer> colIdUtilisateur;
    @FXML
    private TableColumn<OffreEmploi, Void> colActions;

    @FXML
    private TableColumn<CritereOffre, Void> colCritereActions;

    @FXML
    private TableView<CritereOffre> critereTable;
    @FXML
    private TableColumn<CritereOffre, Integer> colIdCritere;
    @FXML
    private TableColumn<CritereOffre, String> colNiveauExperience;
    @FXML
    private TableColumn<CritereOffre, String> colNiveauEtude;
    @FXML
    private TableColumn<CritereOffre, String> colResponsibilities;
    @FXML
    private TableColumn<CritereOffre, String> colCompetences;
    @FXML
    private TableColumn<CritereOffre, Integer> colIdOffreCritere;
    ...
    @FXML
    private PieChart workPrefPieChart;
    @FXML
    ...
    /**
     * Fetches all distinct user IDs from the offre_emploi table in the
     * database. This method retrieves unique user IDs that have created job
     * offers.
     *
    ...
    public void supprimerOffre() {
        ...
        if (selected == null) {
            showAlert(Alert.AlertType.WARNING, "Aucune sélection", "Sélectionnez une offre a supprimer.");
            return;
        }
    ...
    private OffreEmploi showOffreDialog(OffreEmploi existing) {
        ...
        enhanceBtn.setOnAction(evt -> {
            ...
            if (!aiService.isConfigured()) {
                ...
                alert.setContentText("L'API AI n'est pas configurée.\n\n"
                        ...
                        + "Redémarrez l'application aprés configuration.");
                alert.showAndWait();
                return;
            }
        ...
        scrollPane.setStyle("-fx-background-color: #1a1a2e; -fx-background: #1a1a2e; -fx-border-color: transparent;");
            ...
            if (userId == null) {
                ...
                if (editorText != null && !editorText.trim().isEmpty()) {
                    try {
                        ...
                        if (userId <= 0) {
                            userError.setText("L'ID utilisateur doit etre un nombre positif.");
    ...
    private CritereOffre showCritereDialog(int offreId, String jobTitle) {
        ...
        aiButton.setOnAction(e -> {
            try {
                ...
                if (!aiService.isConfigured()) {
                    showAlert(Alert.AlertType.WARNING, "IA Non Configuré",
                    ...
                    try {
                        ...
                        javafx.application.Platform.runLater(() -> {
                            if (result != null && result.length == 2) {
                                ...
                                showAlert(Alert.AlertType.INFORMATION, "Succés",
                                        "Les responsabilités et compétences ont été générées avec succés !");
                            } else {
    ...
    private CritereOffre showCritereDialogForUpdate(CritereOffre existing) {
        ...
        ButtonType saveButton = new ButtonType("Mettre a jour", ButtonBar.ButtonData.OK_DONE);
        ButtonType cancelButton = new ButtonType("Annuler", ButtonBar.ButtonData.CANCEL_CLOSE);
        dialog.getDialogPane().getButtonTypes().addAll(saveButton, cancelButton);
        ...
        // Title
        Label titleLabel = new Label("Modifier le critère");
        titleLabel.setStyle("-fx-font-size: 22px; -fx-font-weight: bold; -fx-text-fill: white; -fx-alignment: center;");
        VBox.setMargin(titleLabel, new javafx.geometry.Insets(0, 0, 10, 0));
    ...
    /**
     * Refreshes all statistics data and updates charts and cards. Called when
     * the statistics tab is opened or when refresh button is clicked.
     */
    @FXML
    ...
    private void goBackToAdmin() {
        try {
            ...
            System.out.println("✅ Retour vers Administration");
        } catch (Exception e) {
            e.printStackTrace();
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Erreur");
            alert.setContentText("Impossible de retourner à l'administration : " + e.getMessage());
            alert.showAndWait();
        }
    }
